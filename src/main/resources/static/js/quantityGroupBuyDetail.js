...
const fetchAPI = {
    ...
    getDetail: () =>
        api.fetch(`/api/quantityGroupBuy/detail/${PAGE_CONFIG.quantityGroupBuyNo}`, {
            method: 'GET'
        })
            .then(res => {
                if (!res.ok) {
    ...
    joinGroupBuy: (groupBuyNo, myQuantity) =>
        api.fetch(`/api/quantityGroupBuy/join`, {
            method: 'POST',
            headers: {
    ...
    modifyQuantity: (groupBuyParticipantNo, newQuantity) =>
        api.fetch(`/api/quantityGroupBuy/quantityModify/${groupBuyParticipantNo}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
    ...
    cancelParticipation: (groupBuyParticipantNo) =>
        api.fetch(`/api/quantityGroupBuy/cancelParticipant/${groupBuyParticipantNo}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            }
    ...
    forceClose: (groupBuyNo) =>
        api.fetch(`/api/quantityGroupBuy/forcedCreator/${groupBuyNo}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            }
    ...
    stopGroupBuy: (groupBuyNo, reason) =>
        api.fetch(`/api/quantityGroupBuy/cancelCreator/${groupBuyNo}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (joinBtn) {
        joinBtn.addEventListener('click', function() {
            ...
            if (!selectedQuantity) {
                showAlertModal('알림', '수량을 선택해주세요.', 'info');
                return;
            }

            showAlertModal(
                '참여 확인',
                `${selectedQuantity}${currentData.groupBuyDetail.unit}으로 참여하시겠습니까?`,
                'question',
                async () => {
                    try {
                        const response = await fetchAPI.joinGroupBuy(groupBuyNo, selectedQuantity);
                        if (response.success) {
    ...
    if (modifyBtn) {
        modifyBtn.addEventListener('click', function() {
            ...
            showAlertModal(
                '수정 확인',
                `수량을 ${newQuantity}${currentData.groupBuyDetail.unit}으로 수정하시겠습니까?`,
                'question',
                async () => {
                    try {
                        const response = await fetchAPI.modifyQuantity(myGroupBuyParticipantNo, newQuantity);
                        if (response.success) {
                            showAlertModal('수정 완료', response.message, 'success', () => location.reload());
                        } else {
                            showAlertModal('수정 실패', response.message || '수정에 실패했습니다.', 'error');
                        }
                    } catch (error) {
                        console.error('수정 오류:', error);
                        showAlertModal('오류 발생', '수정 중 오류가 발생했습니다.', 'error');
                    }
                }
            );
        });
    ...
    if (creatorActionBtn) {
        creatorActionBtn.addEventListener('click', function() {
            ...
            if (actionType === 'run') {
                showAlertModal(
                    ...
                    async () => {
                        try {
                            const response = await fetchAPI.forceClose(groupBuyNo);
                            if (response.success) {
                                showAlertModal('진행 완료', response.message, 'success', () => location.reload());
                            } else {
                                showAlertModal('진행 실패', response.message || '진행에 실패했습니다.', 'error');
                            }
                        } catch (error) {
                            console.error('진행 오류:', error);
                            showAlertModal('오류 발생', '진행 중 오류가 발생했습니다.', 'error');
                        }
                    }
                );
